                ...
                .addOnCompleteListener(this) { task ->
                    if (task.isSuccessful) {
                        ...
                        user?.let {
                            ...
                            val userDocRef = db.collection("managers").document(uid)
                            val newUser = Manager(fullName, email, phoneNumber, city,0,emptyList(),-1L,"",-1,"manager"
                            )
                            userDocRef.set(newUser)
                                .addOnSuccessListener {
                                    saveUserToSharedPreferences("manager", fullName, email, phoneNumber, city)
